...
> **仓库刚改过名、刚合并过两个组件**——看外部资料前请先读下面的「术语变迁」一节，否则会被过时信息误导。
...
| | 谁来做 | 输出 |
|---|-------|------|
| 副本数应该是多少 | [WVA](../../autoscaling/llm-d-autoscaling/) | `wva_desired_replicas` 指标 |
| **这个请求发给哪个 pod** | **Router (EPP)** | **ext-proc 响应里的 endpoint 地址** |

两者都在 llm-d 项目下、都不做 Pod 落位（那是 [kube-scheduler](../kube-scheduler/) / [Volcano](../volcano/) 的事），可以完全独立阅读。本仓库按问题域把它们分在两个分类：Router 在 [`scheduling/`](../)，WVA 在 [`autoscaling/`](../../autoscaling/)。
...
`llm-d` 组织近期做了大幅重构，**大量现存资料（包括上游自己的部分文档）已经过时**：

| 旧名 / 旧位置 | 现状 |
|--------------|------|
| `llm-d-inference-scheduler` | **改名为 `llm-d-router`**。仓库名、镜像名（`llm-d-router-endpoint-picker`）都变了 |
| `llm-d-kv-cache`（独立仓库） | **KV Indexer 已迁入 `llm-d-router` 的 `pkg/kvcache` + `pkg/kvevents`** |
| `llm-d-routing-sidecar`（独立仓库） | **已迁入 `llm-d-router` 的 `pkg/sidecar`**，二进制叫 `pd-sidecar` |
| `gateway-api-inference-extension`（GIE） | 现在**只定义 `InferencePool` API 与 Endpoint Picker 协议**，EPP 实现在 llm-d 侧 |
| `Inference Scheduler` 这个称呼 | 上游文档里仍有残留，指的就是现在的 Router |

**判断资料是否过时的三个信号**：提到 `llm-d-inference-scheduler` 仓库、让你单独部署 `llm-d-kv-cache`、让你单独部署 `routing-sidecar` —— 三者都说明资料在重构之前。

## 版本对照与复现

本系列按 **`main` 分支**分析，固定到核对时的提交 `90a28bc66f1d96f84f8f18f11dcd6ed15f34e830`。基线 `go.mod` 为 **Go 1.26.6**，2872 个可达提交，`pkg/` + `cmd/` 非测试 Go 源码 **78742 行**。

| Git 引用 | 提交 | 与本系列的关系 |
|----------|------|--------------|
| `v0.9.0` tag | — | **不可用**：`pkg/` 下只有 `common`/`epp`/`generator`/`sidecar`，**没有 `kvcache`、`kvevents`、`coordinator`**（04 与 06 篇的内容不存在） |
| `release-0.10` | `71f4f09`（2026-08-17） | 已含 `pkg/kvcache` 与 `pkg/coordinator`，但比 `main` 少 179 个提交、且有 4 个提交不在 `main` 上 |
| **`main`** | **`90a28bc`（2026-09-07）** | **本系列基线** |

**为什么不用 tag**：KV Indexer 与 Coordinator 的迁入都发生在 `v0.9.0` 之后。用 tag 会让 04 篇（KV 索引）和 06 篇（Coordinator 部分）无从谈起。这与本仓库对 vLLM 的处理一致（也是钉 `main` 的核对提交）。

**代价要说清楚**：`main` 会前移，本系列的行号引用会逐渐漂移。逐行复现时请检出固定提交：
...
| `pkg/coordinator` | 4982 | 实验性的独立编排服务（替代 sidecar 的方向） | 06 §8 |
...
| 篇 | 主题 | 核心问题 |
|----|------|---------|
| [00](00-总览与架构.md) | 总览与架构 | Router 在 llm-d 里的位置、**术语变迁与项目边界**、代码地图、四条设计哲学（**万物皆插件**、决策发生在 RequestBody EoS 之后、Filter/Scorer/Screener 的分工、Primary Profile 定去向而其余走 header）、部署模式对照 |
...
```bash
cd sources/llm-d-router
git log --oneline --follow pkg/kvcache/kvblock/index.go     # KV 索引迁入前后的历史
git log --oneline v0.9.0..90a28bc -- pkg/epp/flowcontrol    # Flow Control 这一版改了什么
git diff origin/release-0.10..90a28bc -- pkg/coordinator    # Coordinator 的演进
ls deploy/config/                                            # 19 个配置示例
```

---

> **相关系列**：[llm-d WVA](../../autoscaling/llm-d-autoscaling/)（决定该有几个副本）｜ [kube-scheduler](../kube-scheduler/) / [Volcano](../volcano/)（决定 Pod 落在哪个节点）
> **回到** [调度与编排总览](../README.md)
