...
插件通过 `EnqueueExtensions` 声明「我关心哪些事件」，并为每个事件提供一个 **hint 函数**判断「这个具体的事件对当前这个 Pod 有意义吗」。接着上面的例子，看 `NodeResourcesFit` 怎么做：
...
func (f *Fit) EventsToRegister(_ context.Context) ([]fwk.ClusterEventWithHint, error) {
    ...
    if f.enableInPlacePodVerticalScaling {
        // a Pod update may free up resources that make other Pods schedulable.
    ...
    if f.enableSchedulingQueueHint {
        ...
        nodeActionType = fwk.Add | fwk.UpdateNodeAllocatable      // ★ 开了 QHint 后订阅范围反而更窄
    }
...
func (f *Fit) isSchedulableAfterPodScaleDown(targetPod, originalPod, modifiedPod *v1.Pod) bool {
    if modifiedPod.Spec.NodeName == "" {
        ...
        return false                       // ★ 没落到节点上的 Pod，缩容也不释放资源
    }
    ...
    ...
    for rName, rValue := range podRequests {
        if rValue.IsZero() {
            ...
            continue                       // ★ 我没请求这种资源 → 不关心
        }
        switch rName {
        case v1.ResourceCPU:
            if originalMaxResourceReq.MilliCPU > modifiedMaxResourceReq.MilliCPU {
                return true                // CPU 确实释放了 → 值得重试
            }
        ...
        }
    }
    return false                           // ★ 释放的不是我缺的 → 别唤醒
}
```

回到 §2.1 的例子：Pod-A 缺 CPU，此时别人释放了内存 → `QueueSkip`，Pod-A 不会被无谓唤醒。
