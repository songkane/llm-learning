...
>
> **§6 是一个完整 trace** —— 用统一示例的 vLLM Pod 走完全部扩展点，手工算出每个插件的分数。如果你只想快速建立直觉，可以先跳到 §6 再回头看前面的定义。
>
...
## 6. 完整 trace：一个 vLLM Pod 走完全部扩展点

前面五节是「零件说明书」，这一节把零件装起来跑一遍 —— 用 [00 篇 §1.1 的统一示例](00-kube-scheduler总览与架构.md#11-统一示例三系列共用)，**手工推演每个扩展点的输入、输出、以及具体分数**。

这一节的价值不只是串联流程，还会暴露三个**违反直觉但真实存在**的默认行为（§6.10 汇总）。

### 6.1 场景设定

集群（沿用统一示例的 8 台 8 卡 A100）：

| 节点 | GPU 空闲 | CPU 空闲 | 内存空闲 | 备注 |
|------|---------|---------|---------|------|
| node-1 ~ node-5 | **0 / 8** | 64 / 96 | 512 / 768 Gi | 各被作业 T 的一个 Pod 占满 8 卡 |
| node-6 | 8 / 8 | 96 / 96 | 768 / 768 Gi | **已预热 vLLM 镜像（10 GiB）** |
| node-7 | 8 / 8 | 96 / 96 | 768 / 768 Gi | 无镜像 |
| node-8 | 8 / 8 | 96 / 96 | 768 / 768 Gi | 污点 `maintenance=true:NoSchedule` |

全部节点带 label `accelerator=nvidia-a100`。现在提交**作业 S 的副本 0**（vLLM，TP=4，单 Pod 用 4 张卡）：
...
spec:
  nodeSelector:
    accelerator: nvidia-a100          # ← 注意：这不会裁剪节点集，见 §6.3
  ...
  - name: model-cache
    persistentVolumeClaim:
      claimName: model-cache-0         # StorageClass: WaitForFirstConsumer
  containers:
  - name: vllm
    image: vllm/vllm-openai:latest     # ~10 GiB
    resources:
      limits:
        nvidia.com/gpu: 4
        cpu: 16
...
| `NodeResourcesFit` | `nil, Success` | 把 `{cpu:16, memory:128Gi, nvidia.com/gpu:4}` 算好写入 CycleState |
| `PodTopologySpread` | `nil, Success` | 统计 `app=vllm-qwen` 在各 hostname 的现有数量（全为 0） |
| `InterPodAffinity` | `nil, Skip` | Pod 无 affinity 配置 → **跳过**，后续 Filter/Score 也不跑它 |
| `VolumeBinding` | `nil, Success` | 发现 `model-cache-0` 是 WaitForFirstConsumer，标记「待绑定」 |

**第一个反直觉点**：很多人以为 `nodeSelector: accelerator=nvidia-a100` 会让框架只考虑带该标签的节点。**不会。** `NodeAffinity` 的 `PreFilter` 只在一种情况下返回节点子集 —— Pod 用了 `matchFields: metadata.name`：
...
8 个节点并行跑（`parallelism` 默认 16，节点数不足则按节点数），每个节点内部串行短路（§5.1）：

| 节点 | 结果 | 拒绝插件 | Status Code | 原因 |
|------|------|---------|-------------|------|
| node-1 ~ node-5 | ❌ | `NodeResourcesFit` | `Unschedulable` | `Insufficient nvidia.com/gpu` |
| node-6 | ✅ | — | — | — |
| node-7 | ✅ | — | — | — |
| node-8 | ❌ | `TaintToleration` | `UnschedulableAndUnresolvable` | 不容忍 `maintenance=true` |

两种失败码的区别在这里体现得很清楚（§2）：

- node-1~5 是 `Unschedulable` —— **资源不够，抢占也许有用**，会作为抢占候选；
- node-8 是 `UnschedulableAndUnresolvable` —— **杀谁都没用**（污点不会因为驱逐 Pod 而消失），抢占阶段直接排除。

**可行节点 = [node-6, node-7]。**

### 6.5 PostFilter：本例不跑

`RunPostFilterPlugins` 只在**可行节点数为 0** 时触发（§4.2）。本例有 2 个可行节点，直接跳过。

> 假如作业 T 把 8 台机器全占满，这里就会进 `DefaultPreemption`：把 node-1~5 作为候选（node-8 因 `UnschedulableAndUnresolvable` 被排除），模拟驱逐低优先级 Pod。详见 [03 篇 §4](03-核心代码分析-过滤打分与抢占.md)。

### 6.6 PreScore + Score：算出具体分数

只对 2 个可行节点打分。先看**每个插件的原始分（0~100）**是怎么算出来的。
...
注意 `requested` **已包含待调度 Pod 的请求**。两个节点都空闲，算式相同：

```text
cpu    : (96 - 16) × 100 / 96   = 83     （整数除法）
memory : (768 - 128) × 100 / 768 = 83
nodeScore = (83×1 + 83×1) / 2   = 83
```

**第二个反直觉点：GPU 没有参与打分。** 默认打分资源集只有 CPU 和内存：
...
要让 GPU 参与，必须显式配 `scoringStrategy.resources`（见 [04 篇 §2.1](04-面向大模型场景的能力与局限.md)）。**不配的话，GPU 集群的装箱决策实际上是被 CPU/内存牵着走的。**
...
```text
放之后：f_cpu = 16/96 = 0.1667,  f_mem = 128/768 = 0.1667
        std = |0.1667 - 0.1667| / 2 = 0    → scoreWithPod    = 100
放之前：f_cpu = 0,               f_mem = 0
        std = 0                            → scoreWithoutPod = 100
score = 50 + (50 + 100 - 100) / 2 = 75
```

得 75 —— 正好命中源码注释里说的「均衡度没变化就是 75 左右」。这个 Pod 的 CPU:内存 比例（16:128Gi = 1:8）与节点比例（96:768Gi = 1:8）完全一致，所以不改变均衡度。
...
```text
node-6（有 10 GiB 镜像，8 个节点中只有 1 个有）：
    spread = 1/8 = 0.125
    scaled = 10240 MiB × 0.125 = 1280 MiB
    1280 > maxThreshold(1000×1) → 截断到 1000
    score = 100 × (1000 - 23) / (1000 - 23) = 100
...
两个节点都没有 `PreferNoSchedule` 污点，也都没有 `preferredDuringSchedulingIgnoredDuringExecution`，所以两个插件的**原始分都是 0**。但归一化后结果完全不同：
...
`app=vllm-qwen` 目前一个 Pod 都没有，所有节点原始分相同，走它自己的 `NormalizeScore` 的特殊分支：
...
两个节点都是 100。**副本 1 提交时才会真正起作用** —— 那时 node-6 已有 1 个同标签 Pod，它的 skew 更大、得分更低，副本 1 就会被推向 node-7，实现「两个副本分散在不同机器」。

#### 加权汇总

权重来自默认配置（§5.3）：

| 插件 | 权重 | node-6 原始 | node-7 原始 | node-6 加权 | node-7 加权 |
|------|-----|------------|------------|------------|------------|
| `TaintToleration` | 3 | 100 | 100 | 300 | 300 |
| `NodeAffinity` | 2 | 0 | 0 | 0 | 0 |
| `PodTopologySpread` | 2 | 100 | 100 | 200 | 200 |
| `InterPodAffinity` | 2 | *Skip* | *Skip* | — | — |
| `NodeResourcesFit` | 1 | 83 | 83 | 83 | 83 |
| `NodeResourcesBalancedAllocation` | 1 | 75 | 75 | 75 | 75 |
| `ImageLocality` | 1 | **100** | **0** | **100** | **0** |
| **总分** | | | | **758** | **658** |

**结论：100 分的差距全部来自 ImageLocality。** 在这个 GPU 场景里，八项打分中有五项对两个节点给出完全相同的分数，真正做决定的是「谁提前拉过镜像」—— 而**卡数、卡的空闲程度、GPU 碎片情况，一项都没参与**。

这正是 04 篇要改 `scoringStrategy` 的原因。
...
这个差异的实际影响：大批相同规格的 Pod（比如推理服务扩容 20 个副本）在同分节点之间**可能倾向于同一个节点**，而不是均匀铺开。如果需要打散，别指望这层随机性，应该用 `PodTopologySpread`（本例就配了）。
...
| 步骤 | 动作 |
|------|------|
| `assume` | 缓存中把 Pod 记为「已在 node-6 上」，node-6 的 GPU 空闲从 8 变 4。**下一个 Pod 立刻看到这个变化** |
| `Reserve` | `VolumeBinding` 在内存里预留 `model-cache-0` 与 node-6 上 PV 的配对；`DynamicResources` 无 claim 则跳过 |
| `Permit` | 无 `GangScheduling`（Alpha 未开）→ 直接 `Success`，`pluginsWaitTime = 0` |

**Reserve 必须快**（只改内存）—— 它还在串行的调度周期里，慢一毫秒全集群吞吐就掉一毫秒。真正的慢操作留给 `PreBind`。

> 如果开了 `GangScheduling` gate 且这是个 gang 的一部分，`Permit` 会返回 `Wait` —— 而此时 **assume 已经发生**，4 张卡已经被占着了。这就是 [00 篇 §6.3](00-kube-scheduler总览与架构.md) 说的原生 gang 与 Volcano 事务式 gang 的本质差异。
...
失败的话：`Unreserve` 回滚（释放 PV 预留）+ `Cache.ForgetPod()`（释放 assume 的 4 张卡）+ Pod 回到队列。
...
sequenceDiagram
    ...
    Q->>SC: NextPod() → vllm-qwen-0
    SC->>C: UpdateSnapshot()
    Note over SC: PreFilter：8 节点全保留<br/>（nodeSelector 不裁剪）
    Note over SC: Filter：node-1~5 GPU 不足<br/>node-8 污点 → 剩 node-6/7
    Note over SC: Score：758 vs 658<br/>（差距全来自 ImageLocality）
    SC->>C: assume(node-6)：GPU 8→4
...
| **GPU 默认不参与打分**（`defaultResourceSpec` 只有 cpu/memory） | GPU 碎片化，8 卡机器被 1 卡 Pod 占位 | 显式配 `scoringStrategy.resources` 加 `nvidia.com/gpu`（[04 篇 §2.1](04-面向大模型场景的能力与局限.md)） |
| **同分不随机**（`Randomizer` 仅 Extender 路径赋值） | 相同 Pod 批量扩容可能挤向同一节点 | 用 `PodTopologySpread` 显式打散 |

再加一条容易被老资料误导的：**`NodeResourcesBalancedAllocation` 现在算的是「均衡度的变化量」**，比例匹配的 Pod 恒得 75 分，不再是「1 - 方差」。
...
| 「GPU 会参与节点打分」 | ❌ `defaultResourceSpec` 只有 cpu / memory，必须显式配 `scoringStrategy.resources`（§6.6） |
