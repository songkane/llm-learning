...
它的形态是**单仓多组件**：Frontend、Router、Planner、KVBM、Operator 住在同一个仓库里，共享同一套运行时。Rust 写核心（`lib/`，22 个 crate），Python 做扩展层（PyO3 / maturin 绑定）。1.x 起 **etcd 和 NATS 都不再是必需依赖**。
...
```
发现平面的 KV key（discovery/mod.rs:936）    {ns}/{component}/{endpoint}/{instance_id:x}
请求平面的 TCP 地址（component/endpoint.rs:337）  {host}:{port}/{connection_id:x}/{endpoint_name}
```
...
一个容易被忽略但很实用的机制：**local worker inhibition**。请求失败后，本地 runtime 会临时「抑制」那个 worker，给发现平面追上来的时间，默认 5 秒（`DYN_RUNTIME_INHIBITED_DURATION_SECS`）。发现平面仍然是权威——它可以在计时器到期前就恢复或彻底移除该 worker。
