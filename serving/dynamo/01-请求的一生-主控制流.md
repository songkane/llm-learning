...
| 交互式命令行 | `-i` | `main.py:483` |
| KServe gRPC | `--kserve-grpc-server` | `main.py:487` |

四种起法共用同一套装配代码，区别只在最外层的 "input" 是 HTTP、stdin 还是 gRPC。这个设计的好处在 06 篇会体现：GAIE 场景下 Frontend 要作为 **sidecar 塞进 decode pod**，靠的就是它能被嵌入。
...
input.rs:99   run_input(runtime, "http", engine)
     ↓
input/http.rs:31   HttpFrontend::run()
     ↓                      ├── 起 axum HTTP server
     ↓                      └── 起 ModelWatcher（后台任务）
```
...
Frontend 侧
   input/http.rs:368   ModelWatcher 调 list_and_watch(AllModels)
   discovery/watcher.rs:443~745   收到新模型 → 现场建 pipeline
       watcher.rs:464   drt.namespace(ns)?.component(comp)?.endpoint(ep)
       watcher.rs:466   endpoint.client()
```
...
```490:533:lib/llm/src/entrypoint/input/common.rs
...
四个算子在**没启用对应特性时是 pass-through**（`EncoderRouter::disabled()`、`PrefillRouter::disabled_with_selector()`，见 `common.rs:304、310`），所以聚合部署下这条链实际只有 preprocessor、migration、backend 三段有活干。
...
`backend_engine` 到底是什么？`common.rs:314~328` 给了答案：

```314:328:lib/llm/src/entrypoint/input/common.rs
...
```192:203:lib/runtime/src/pipeline/network/egress/push_router.rs
...
前五种走 `PushRouter::generate`。后两种不走——`push_router.rs:2031` 直接 `bail!("KV routing should not call generate")`，因为 KV 和 Direct 的选择逻辑在 `RoutingHost` 里。
...
1. **Frontend 是库不是服务**，四种起法共用一套装配代码；GAIE sidecar 形态靠的就是这一点。
