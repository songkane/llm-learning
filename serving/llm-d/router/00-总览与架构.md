...
> **源码基线**：[`main @ 90a28bc`](https://github.com/llm-d/llm-d-router/tree/90a28bc66f1d96f84f8f18f11dcd6ed15f34e830)（2026-09-07 提交，2026-09-08 核对），见 [版本与复现](README.md#版本与复现)。
> 本地路径：`sources/llm-d-router`（`./scripts/sync-sources.sh llm-d-router`）
> Go module：`github.com/llm-d/llm-d-router`。
...
按上游 `docs/architecture.md` 的划分，llm-d 的核心是三个概念：
...
**GIE 与 llm-d-router 的分工**：GIE 只定义 `InferencePool` API 与 Endpoint Picker Protocol（协议与 CRD），EPP 的实现全在 `llm-d-router`。此外 `InferenceObjective`、`InferenceModelRewrite` 等 API 类型定义在本仓库的 `apix/`。

**一个仓库装下四件事**：EPP（`pkg/epp`）、KV 索引（`pkg/kvcache` + `pkg/kvevents`）、P/D sidecar（`pkg/sidecar`）、实验性 Coordinator（`pkg/coordinator`）都在 `llm-d-router` 里，不需要单独部署额外仓库的组件。

**Variant（变体）** 是 InferencePool 内部用 pod label 表达的逻辑子分组（如 `role=prefill` / `role=decode`），**不是独立的 K8s 资源**——这点和 WVA 笔记里的「变体」是同一个概念，见 [llm-d WVA 系列](../autoscaling/README.md)。

### 可选叠加的组件

| 模式 | 仓库 | 与本系列的关系 |
|------|------|----------------|
| KV Cache 管理 | `llm-d-kv-cache` | 只提供 PVC Evictor；**路由用的索引逻辑在本仓库，见 04 篇** |
...
InferencePool: llama-8b-pool（selector: app=llama-8b）
  ├── decode  pod  D1..D4   label: llm-d.ai/role=decode（各含一个 pd-sidecar）
  └── prefill pod  P1..P2   label: llm-d.ai/role=prefill
所有 pod 都打 llm-d.ai/engine-type=vllm

Gateway（Envoy）+ 一个 EPP Deployment

系统提示 S：2048 token（Go 工程规范 + few-shot 示例），A/B 完全相同
请求 A: S + "请解释 sync.Map 的适用场景。"       共 2048 + 40 = 2088 token
请求 B: S + "请解释 channel 的关闭语义。"        共 2048 + 24 = 2072 token
        ↑ A 与 B 共享 2048 token 的前缀
```

**为什么把系统提示定成 2048 token 而不是随手写一句话**：前缀缓存的匹配粒度是 **block 不是 token**，而近似路线的 block size 有 **64 token 的硬下限**（04 篇 §4.2）。共享前缀短于一个 block 时，前缀路由**在默认配置下完全不生效**——这类示例算出来的"命中"是假的。2048 是 64 和 16 的公倍数，两条路线（近似 / 精确）都能整齐地凑满整数个 block，推演结果才成立。真实的 agentic / 长系统提示场景本来就在这个量级。
...
4. B 请求进来时，EPP 怎么知道 D3 上已经有那 2048 token 前缀的 KV？（04 篇）
...
| **请求入口** | **请求发给哪个 pod** | **本系列** |
| 副本数 | 该开几个 pod | [llm-d WVA](../autoscaling/) |
| Pod 放置 | pod 落到哪个节点 | [kube-scheduler](../../../scheduling/kube-scheduler/)、[Volcano](../../../scheduling/volcano/)、[Kueue](../../../scheduling/kueue/) |
...
- **03 篇的池级聚合指标** ↔ [WVA 的指标采集](../autoscaling/02-核心代码分析-指标采集与Analyzer.md)：WVA 消费的正是 EPP 暴露的这些指标
...
| 键 | 值 | 作用 | 代码 |
|----|----|------|------|
| `llm-d.ai/role` | **六个取值**，见下表 | 角色过滤（`decode-filter` 等） | `framework/plugins/scheduling/filter/bylabel/roles.go:13-25` |
...
**`llm-d.ai/role` 的六个取值，以及三个 role filter 各自接受哪些**（`roles.go:42,52,64`）——注意**不是一对一**，兼职角色会被多个 filter 同时接受：

| label 取值 | `decode-filter` 收 | `prefill-filter` 收 | `encode-filter` 收 |
|-----------|:---:|:---:|:---:|
| `decode` | ✓ | | |
| `prefill` | | ✓ | |
| `encode` | | | ✓ |
...
单体部署（一个 pod 既 prefill 又 decode）打 `prefill-decode`，它会同时进两个 profile 的候选集——**这正是 P/D 与非 P/D 能共用一份 EPP 配置的原因**。E/P/D 场景才需要后两个取值。
...
1. **看核心代码看不出行为，必须配着 YAML 看**。`deploy/config/` 下有 19 个现成的 `EndpointPickerConfig` 示例，`deploy/config/pd-epp-config.yaml` 是最有代表性的一个（07 篇把它和精确前缀缓存的示例合成了一份可部署的完整配置）。
...
3. **`docs/` 与代码不一致时以代码为准**。架构类的 canonical 文档在 `llm-d/llm-d` 主仓库的 `docs/architecture/core/router/`，本仓库 `docs/` 下是面向使用的说明。

```bash
cd sources/llm-d-router
ls deploy/config/                                        # 19 个 EndpointPickerConfig 示例
rg -l 'plugin.Plugin' pkg/epp/framework/interface/       # 八类扩展点的接口定义
find pkg/epp/framework/plugins -name '*.go' ! -name '*_test.go' | sed 's|.*plugins/||' | cut -d/ -f1 | sort -u
```
